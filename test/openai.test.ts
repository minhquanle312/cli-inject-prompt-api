...
import { buildChatCompletionResponse, parseChatCompletionRequest } from "../src/openai.js";
import { buildPrompt } from "../src/prompt.js";

test("parser rejects stream true", () => {
  const parsed = parseChatCompletionRequest({ model: "gemini-3.5-flash", stream: true, messages: [{ role: "user", content: "hi" }] });
  assert.deepEqual(parsed, { status: 400, code: "unsupported_stream", message: "stream=true is not supported by this local proxy" });
});
